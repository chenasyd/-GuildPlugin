...
import type { SlotSize, GuiEntry } from "@/types/imago";
...
export function AddGuiImageDialog({ open, onClose }: Props) {
  const guiFolders = useEditorStore((s) => s.guiFolders);
  const addGuiEntry = useEditorStore((s) => s.addGuiEntry);
  const updateGuiEntryTexture = useEditorStore((s) => s.updateGuiEntryTexture);
  const nextGuiChar = useEditorStore((s) => s.nextGuiChar);

  const [slot, setSlot] = useState<SlotSize>(54);
  ...
  const folder = guiFolders.find((f) => f.slot === slot);
  ...
  const handleSubmit = () => {
    ...
    const character = nextGuiChar();
    const entry: GuiEntry = {
      id: `${slot}:${name}`,
      name: name.trim(),
      slot,
      ascent: useDefaults && folder ? folder.defaults.ascent : ascent,
      height: useDefaults && folder ? folder.defaults.height : height,
      shiftX,
      character,
      textureData: null,
      textureSrc: null,
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        addGuiEntry(entry);
        updateGuiEntryTexture(slot, name.trim(), img);
        onClose();
      };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      ...
      >
        <h3 className="text-base font-medium text-zinc-200 mb-4">Add GUI Image</h3>

        {/* Slot Size */}
        <div className="mb-3">
          <label className="block text-xs text-zinc-400 mb-1">Slot Size</label>
          <select
            value={slot}
            onChange={(e) => setSlot(Number(e.target.value) as SlotSize)}
          ...
          >
            {SLOT_SIZES.map((s) => (
              <option key={s} value={s}>
                {s} Slots ({Math.sqrt(9/9) > 0 ? s / 9 : 1} rows x 9 cols)
              </option>
            ))}
          </select>
        </div>
        ...
        {/* Entry Name */}
        <div className="mb-3">
          <label className="block text-xs text-zinc-400 mb-1">Entry Name</label>
          <input
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="e.g. mainguildgui"
            className="w-full bg-zinc-700 text-zinc-200 border border-zinc-600 rounded px-3 py-1.5 text-sm focus:outline-none focus:border-blue-500"
          />
        </div>
        ...
        <div className="mb-3 flex items-center gap-2">
          ...
          <span className="text-xs text-zinc-400">
            Use folder defaults
            {folder && ` (ascent=${folder.defaults.ascent}, height=${folder.defaults.height})`}
          </span>
        </div>

        {/* Ascent, Height, ShiftX */}
        {!useDefaults && (
          <div className="grid grid-cols-3 gap-2 mb-3">
            ...
            <div>
              <label className="block text-xs text-zinc-400 mb-0.5">Shift X</label>
              <input
                type="number"
                value={shiftX}
