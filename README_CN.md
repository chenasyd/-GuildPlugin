...
> ⚠️ **提示**：如果您使用 v1.3.9 或更高版本，请务必更新插件配置到最新版本；否则插件会出现大量显示错误，影响玩家体验。
...
从 v1.4.x 起，升级费用由配置驱动（不再硬编码）。新增配置项位于 `guild` 下：

- `guild.max-level`：最大等级（整数），达到该等级后工会不再有下一级（默认 `10`）。
- `guild.levels.<n>`：以当前等级 `n` 为键，值为从等级 `n` 升到 `n+1` 所需的资金（浮点数）。
...
插件启动时会读取这些值，并在 GUI（如工会信息页）中显示“下级所需”与进度条。修改配置后可使用 `/guildadmin reload` 进行热加载；若配置缺失，则回退到内置默认值以保证向后兼容。
...
### v1.4.0 (当前版本) - 任务系统模块
- **SDK 覆盖率提升至 100%** (完整覆盖所有公开方法)
- **新增 GuildQuestModule**: 完整的任务系统，支持每日、每周、一次性任务
- **C币奖励系统**: 任务完成后获得 C币奖励
- **实时进度追踪**: 任务进度实时更新
- **任务重置机制**: 每日/每周自动重置任务
- **完整的 CustomGUI 系统**: 任务列表、任务详情、活跃任务列表
- **4个示例模块全部完成**: AnnouncementModule / MemberRankModule / GuildStatsModule / GuildQuestModule

### v1.3.6 - GuildStatsModule 完成
- 新增第三个示例模块：数据统计面板（活跃度/经济报表/排行榜/Web上报）
- SDK 覆盖率从 45% 提升至 82%
...
- [ ] 公会战争系统（部分实现）
- [x] ~~公会公告系统~~ ✅ (AnnouncementModule, v1.3.x)
- [x] ~~公会排行榜~~ ✅ (GuildStatsModule-GuildRankingGUI, v1.3.x)
- [ ] 公会任务/任务系统 **← 下一步: QuestModule**
- [ ] 公会活动系统
- [ ] 公会仓库
- [x] ~~插件扩展模块SDK~~ ✅ (完整SDK + 4示例模块, 覆盖率100%, v1.4.0)
- [ ] 快速获取资源更新
- [ ] 报错快捷反馈
- [ ] 报错提示代码位置或具体问题
- [ ] 插件单独记录详细的日志
