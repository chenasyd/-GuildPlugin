...
### v1.3.9 (当前版本) - SDK 深度完善
- **SDK 覆盖率提升至 96%** (50+ 公开方法)
- **阶段A (v1.3.7)**: 7项改进 — 成员事件、统一消息发送、异步IO、配置驱动化、GUI工具统一、HTTP GET能力、按名查询
- **阶段B (v1.3.8)**: 4项改进 — CustomGUI完整生命周期(注册/打开/注销)、跨GUI导航
- **阶段C (v1.3.9)**: 5项改进 — 延迟初始化、动态语言切换、模块元信息日志、EventBus事件总线、GUI导航栈查询
- **核心修复**: ModuleContext.formatMessage() 统一颜色化处理，解决颜色代码原样输出问题
- **3个示例模块全部完成**: AnnouncementModule / MemberRankModule / GuildStatsModule
...
- [ ] 公会活动系统
- [ ] 公会仓库
- [x] ~~插件扩展模块SDK~~ ✅ (完整SDK + 3示例模块, 覆盖率96%, v1.3.9)
- [ ] 快速获取资源更新
- [ ] 报错快捷反馈
- [ ] 报错提示代码位置或具体问题
- [ ] 插件单独记录详细的日志
