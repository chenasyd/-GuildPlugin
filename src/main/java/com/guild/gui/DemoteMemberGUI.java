...
public class DemoteMemberGUI implements GUI {
    ...
    private void loadMembers() {
        plugin.getGuildService().getGuildMembersAsync(guild.getId()).thenAccept(memberList -> {
            this.members = memberList.stream()
                ...
                .filter(member -> member.getRole().equals(GuildMember.Role.OFFICER)) // Pokaż tylko oficerów
    ...
    public String getTitle() {
        return ColorUtils.colorize("&6Degradacja - Strona " + (currentPage + 1));
    }
    ...
    private ItemStack createMemberHead(GuildMember member) {
        ...
        if (meta != null) {
            ...
            meta.setLore(Arrays.asList(
                ...
                ColorUtils.colorize("&7Kliknij, aby zdegradować do rangi Członek")
            ));
    ...
    private void handleDemoteMember(Player demoter, GuildMember member) {
        ...
        plugin.getGuildService().updateMemberRoleAsync(member.getPlayerUuid(), GuildMember.Role.MEMBER, demoter.getUniqueId()).thenAccept(success -> {
            if (success) {
                String demoterMessage = plugin.getConfigManager().getMessagesConfig().getString("demote.success", "&aZdegradowano &e{player} &ado rangi Członek!")
                    .replace("{player}", member.getPlayerName());
                demoter.sendMessage(ColorUtils.colorize(demoterMessage));

                // Powiadom zdegradowanego gracza
                Player demotedPlayer = plugin.getServer().getPlayer(member.getPlayerUuid());
                if (demotedPlayer != null) {
                    String demotedMessage = plugin.getConfigManager().getMessagesConfig().getString("demote.demoted", "&cZostałeś zdegradowany do rangi Członek w gildii &e{guild}&c!")
                ...
                String message = plugin.getConfigManager().getMessagesConfig().getString("demote.failed", "&cDegradacja członka nie powiodła się!");
                demoter.sendMessage(ColorUtils.colorize(message));
            }
        });
    }
