...
public class ConfirmDemoteMemberGUI extends AbstractConfirmGUI {

    private final GuildMember member;

    public ConfirmDemoteMemberGUI(GuildPlugin plugin, Guild guild, GuildMember member, Player player, String sourceGuiType) {
        super(plugin, guild, player, sourceGuiType != null ? sourceGuiType : "DemoteMemberGUI");
        this.member = member;
    }
    ...
    protected String titleKey() {
        return "gui.confirm-demote-member.title";
    }

    @Override
    protected String titleDefault() {
        return "&7Confirm Demote Member";
    }

    @Override
    protected String bedrockTitleKey() {
        return "gui.confirm-demote-member.bedrock-title";
    }
    ...
    protected String bedrockContentKey() {
        return "gui.confirm-demote-member.bedrock-content";
    }
    ...
    protected String bedrockConfirmKey() {
        return "gui.confirm-demote-member.bedrock-confirm";
    }

    @Override
    protected String bedrockConfirmDefault() {
        return "&7Confirm Demote";
    }

    @Override
    protected String bedrockCancelKey() {
        return "gui.confirm-demote-member.bedrock-cancel";
    }
    ...
    protected String confirmButtonKey() {
        return "gui.confirm-demote-member.confirm-button";
    }
    ...
    protected String confirmLoreKey() {
        return "gui.confirm-demote-member.confirm-lore";
    }

    @Override
    protected String confirmLoreDefault() {
        return "&7Click to confirm demotion";
    }

    @Override
    protected String cancelButtonKey() {
        return "gui.confirm-demote-member.cancel-button";
    }
    ...
    protected String cancelLoreKey() {
        return "gui.confirm-demote-member.cancel-lore";
    }
        ...
        plugin.getGuildService().getGuildMemberAsync(guild.getId(), player.getUniqueId()).thenAccept(executor -> {
            CompatibleScheduler.runTask(plugin, player, () -> {
                ...
                if (member.getRole() == GuildMember.Role.LEADER) {
                    player.sendMessage(ColorUtils.colorize(languageManager.getGuiMessage(player,
                            "gui.confirm-demote-member.cannot-demote-leader", "&cCannot demote the guild leader!")));
                    return;
                }
                if (member.getRole() != GuildMember.Role.OFFICER) {
                    player.sendMessage(ColorUtils.colorize(languageManager.getGuiMessage(player,
                            "gui.confirm-demote-member.not-officer", "&cThat member is not an officer!")));
                    return;
                }
                ...
                plugin.getGuildService()
                        ...
                        .thenAccept(success -> CompatibleScheduler.runTask(plugin, player, () -> {
                            if (success) {
                                ...
                                player.sendMessage(ColorUtils.colorize(languageManager.getGuiMessage(player,
                                        "gui.confirm-demote-member.demote.failed", "&cFailed to demote member!")));
                            }
                        }));
            });
        });
